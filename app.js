...
const APP_VERSION = '20260507-save-timeout-1';
...
async function withRequestTimeout(label, task){
  await ensureFreshSession();
  return await withTimeout(label, task, REQUEST_TIMEOUT_MS);
}
...
async function saveLaptop(event){
  ...
  const editingExistingLaptop = Boolean(currentEditId);
  isSavingLaptop = true;
  if(saveBtn){
  ...
  try{
    if(currentEditMode === 'location' && currentEditId){
      ...
      const response = await runWithReconnect(
        'Save location',
    ...
    const payload = {
      ...
      model_type: document.getElementById('charger_cost') ? getSelectedModelType() : '',
    ...
    let response = await runWithReconnect(
      ...
      { retryThrown: Boolean(currentEditId) }
    );
    ...
    if(response.error && isMissingModelTypeColumnError(response.error) && Object.prototype.hasOwnProperty.call(payload, 'model_type')){
      ...
      response = await runWithReconnect(
        ...
        () => currentEditId
          ? supabaseClient.from(TABLE).update(fallbackPayload).eq('id', currentEditId).select().single()
