...
const APP_VERSION = '20260507-pending-save-queue-1';
...
async function saveLaptop(event){
  ...
  try{
    if(currentEditMode === 'location' && targetEditId){
      ...
      queueLaptopPatch(targetEditId, payload);
      closeAddModal();
      resetForm();
      resetSaveButton(saveBtn, saveWatchdog);
      setBanner('Збережено локально. Синхронізую з базою у фоні.');
      processPendingSaves();
      return;
    }
    ...
    if(targetEditId){
      const currentItem = laptops.find((x) => x.id === targetEditId);
      const optimisticItem = { ...(currentItem || {}), ...payload, id: targetEditId };
      queueLaptopPatch(targetEditId, payload);
      applyLaptopToState(optimisticItem);
      closeAddModal();
      resetForm();
      resetSaveButton(saveBtn, saveWatchdog);
      setBanner('Збережено локально. Синхронізую з базою у фоні.');
