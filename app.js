...
const APP_VERSION = '20260507-direct-rest-save-debug-1';
...
async function saveLaptop(event){
  ...
  if(saveBtn){
    ...
    if(editingExistingLaptop) saveWatchdog = window.setTimeout(() => {
      ...
      saveBtn.disabled = false;
      saveBtn.textContent = 'Зберегти';
      setBanner('Збереження зависло. Перевір інтернет і натисни “Зберегти” ще раз.', false);
    }, SAVE_UI_TIMEOUT_MS);
  }
  ...
  try{
    if(currentEditMode === 'location' && targetEditId){
      ...
      const response = await updateLaptopDirect(targetEditId, payload, 'Save location direct');
      if(response.error){
        hasSupabaseConnection = false;
        updateNetwork();
        console.error(response.error);
        setBanner(`Помилка збереження локації: ${errorSummary(response.error)}`, false);
        return;
      }
      hasSupabaseConnection = true;
      updateNetwork();
    ...
    if(!payload.number){
      setBanner('Введи номер ноутбука.', false);
      return;
    }
    ...
    if(targetEditId){
      ...
      if(currentItem && !allowed[currentItem.status]?.includes(payload.status)){
        setBanner('Недозволена зміна статусу.', false);
        return;
      }

      if(payload.status === 'sold' && (!payload.sold_price || Number(payload.sold_price) <= 0)){
        setBanner('Введи ціну продажу перед статусом Продано.', false);
        return;
      }
    ...
    let response = targetEditId
      ? await updateLaptopDirect(targetEditId, payload, 'Save laptop direct')
    ...
    if(response.error && isMissingModelTypeColumnError(response.error) && Object.prototype.hasOwnProperty.call(payload, 'model_type')){
      ...
      response = targetEditId
        ? await updateLaptopDirect(targetEditId, fallbackPayload, 'Save laptop direct without model type')
        : await runWithReconnect(
    ...
    if(response.error){
      ...
      updateNetwork();
      console.error(response.error);
      setBanner(`Помилка збереження в базу: ${errorSummary(response.error)}`, false);
      return;
    }
    hasSupabaseConnection = true;
    updateNetwork();
    ...
    if(message.includes('timed out')){
      setBanner('Збереження не відповіло за таймаут. Натисни “Зберегти” ще раз.', false);
    } else {
      setBanner(`Помилка збереження: ${errorSummary(error)}`, false);
    }
  }finally{
    resetSaveButton(saveBtn, saveWatchdog);
  }
...
function bindUI(){
  ...
  document.addEventListener('change', (event) => {
    if(event.target.id === 'filterStatus' || event.target.id === 'filterMarket' || event.target.id === 'filterModelType' || event.target.id === 'filterTracking'){
      renderActive();
    }
