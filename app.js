...
function renderDashboardDeliveryOptions(){
  ...
  dashboardDeliveryNoteValue = dashboardDeliveryNoteValue.filter((number) => activeLaptops.includes(number));
  ...
  listWrap.innerHTML = activeLaptops.map((number) => `
    <label class="dashboard-note-option">
      <input type="checkbox" value="${safe(number)}" ${dashboardDeliveryNoteValue.includes(number) ? 'checked' : ''} />
      <span>${safe(number)}</span>
    </label>
  `).join('');
