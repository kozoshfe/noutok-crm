  ...
  try{
    if(currentEditMode === 'location' && currentEditId){
      ...
      const response = await runWithReconnect('Save location', () =>
        supabaseClient.from(TABLE).update(payload).eq('id', currentEditId)
      );
      if(response.error){
        hasSupabaseConnection = false;
        updateNetwork();
...
async function quickStatus(id, status){
  ...
  const { error } = await runWithReconnect('Quick status', () =>
  ...
  if(error){
    ...
    updateNetwork();
    console.error(error);
    alert(`Не вдалося змінити статус: ${error.message || 'невідома помилка'}`);
    return;
  }
