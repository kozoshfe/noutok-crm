...
const APP_VERSION = '20260507-model-no-focus-1';
  ...
  try{
    if(currentEditMode === 'location' && targetEditId){
      ...
      const response = await runWithReconnect(
        ...
        (signal) => attachAbortSignal(
          supabaseClient.from(TABLE).update(payload).eq('id', targetEditId),
          signal
        ),
        { retryThrown: false }
      );
    ...
    if(response.error && isMissingModelTypeColumnError(response.error) && Object.prototype.hasOwnProperty.call(payload, 'model_type')){
      ...
      response = await runWithReconnect(
        ...
        (signal) => attachAbortSignal(
          targetEditId
            ? supabaseClient.from(TABLE).update(fallbackPayload).eq('id', targetEditId)
