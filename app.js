...
function renderStats(){
  const active = laptops.filter((x) => x.status !== 'sold');
  const sold = laptops.filter((x) => x.status === 'sold');
        ...
        <div class="sold-card">
          <div class="sold-card-top">
            ...
            <div class="sold-card-side">
              <div class="badge sold-card-status st-${safe(item.status)}">${safe(statusLabels[item.status] || item.status)}</div>
      ...
      <div style="padding:16px;display:flex;flex-direction:column;gap:12px">
        <div class="active-card-top">
          ...
          <div class="active-card-side">
            ...
            <div class="badge active-card-status st-${safe(item.status)}">${safe(statusLabels[item.status] || item.status)}</div>
          </div>
        </div>
      </div>
    </div>
...
function renderActive(){
  ...
  let data = laptops.filter((x) => x.status !== 'sold');
...
function renderSold(){
  const data = laptops.filter((x) => x.status === 'sold');
      ...
      <div class="location-card">
        <div class="location-card-header">
          <div class="location-card-meta">
            ...
            ${item.location ? `<div class="location-card-badge">${safe(item.location)}</div>` : ''}
          </div>
          <button class="edit-mini location-card-edit" onclick="openEditModal('${item.id}', 'location')" title="Редагувати локацію">✏️</button>
        </div>
      </div>
    </div>
  `).join('') : '<div class="empty">Немає активних ноутбуків</div>';
}

function renderMonths(){
  const sold = laptops.filter((x) => x.status === 'sold' && x.sold_at);
...
function toggleAdditionalCostsFields(clearOnHide = false){
  ...
  if(!checkbox.checked && clearOnHide){
    ['duty_cost', 'ssd', 'ram'].forEach((id) => {
...
function updateCostDisplay(){
  ...
  const draft = {
    ebay_price: document.getElementById('ebay_price')?.value || 0,
    delivery_cost: document.getElementById('delivery_cost')?.value || 0,
    charger_cost: document.getElementById('charger_cost')?.value || 0,
    duty_cost: document.getElementById('duty_cost')?.value || 0,
    olx_ad_cost: document.getElementById('olx_ad_cost')?.value || 0,
    engraving_cost: document.getElementById('engraving_cost')?.value || 0,
    ssd: document.getElementById('ssd')?.value || 0,
    ram: document.getElementById('ram')?.value || 0
  };
  display.value = calcCost(draft);
}
...
function openEditModal(id, mode = 'full'){
  ...
  if(!extra){
    ...
    extra.innerHTML = `
      <div class="form-grid" style="margin-top:12px">
        ...
        <div class="span-3 additional-toggle-row">
          <label class="checkbox-chip">
            <input id="hasAdditionalCosts" type="checkbox" />
        ...
        <div id="additionalCostsFields" class="span-3 form-grid nested-grid">
  ...
  const hasAdditionalCosts = document.getElementById('hasAdditionalCosts');
  if(hasAdditionalCosts){
    hasAdditionalCosts.checked = toNum(item.duty_cost) > 0 || toNum(item.ssd) > 0 || toNum(item.ram) > 0;
    if(!hasAdditionalCosts.dataset.boundToggle){
      hasAdditionalCosts.addEventListener('change', () => toggleAdditionalCostsFields(true));
      hasAdditionalCosts.dataset.boundToggle = '1';
    }
  }
  toggleAdditionalCostsFields(false);

  ['ebay_price', 'delivery_cost', 'charger_cost', 'duty_cost', 'ssd', 'ram'].forEach((id) => {
    const input = document.getElementById(id);
    if(input && !input.dataset.boundCost){
      input.addEventListener('input', updateCostDisplay);
      input.dataset.boundCost = '1';
    }
  });
...
async function saveLaptop(event){
  ...
  const hasAdditionalCosts = document.getElementById('hasAdditionalCosts');
  if(hasAdditionalCosts && !hasAdditionalCosts.checked){
    payload.duty_cost = 0;
    payload.ssd = 0;
    payload.ram = 0;
  }
