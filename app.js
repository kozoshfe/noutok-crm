...
function loadDashboardDeliveryNote(){
  const input = document.getElementById('dashboardDeliveryNote');
  if(!input) return;
  input.value = localStorage.getItem(DASHBOARD_DELIVERY_NOTE_KEY) || '';
}

function saveDashboardDeliveryNote(){
  const input = document.getElementById('dashboardDeliveryNote');
  if(!input) return;
  const value = String(input.value || '').trim();

  if(value){
    localStorage.setItem(DASHBOARD_DELIVERY_NOTE_KEY, value);
  } else {
    localStorage.removeItem(DASHBOARD_DELIVERY_NOTE_KEY);
  }

  setBanner('Нотатку на головній збережено.', true);
}
...
async function quickStatus(id, status){
  ...
  if(status === 'received') payload.location_state = 'На чистку';
...
function bindUI(){
  ...
  if(dashboardDeliveryNote && !dashboardDeliveryNote.dataset.bound){
    dashboardDeliveryNote.addEventListener('change', saveDashboardDeliveryNote);
